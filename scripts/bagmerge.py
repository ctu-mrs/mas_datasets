...
def get_next(bag_iter, reindex = False, 
    ...
    try:
        ...
        if reindex:
            ts = None
            if result[0] == "/tf" and result[1].transforms:
                ts = result[1].transforms[0].header.stamp
            elif result[1]._has_header:
                ts = result[1].header.stamp 
            else: 
                ts = result[2] - start_time + main_start_time
