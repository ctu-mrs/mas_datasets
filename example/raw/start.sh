...
# change this to one of the dataset names {rectangle, loop, forward, lateral, vertical, hover}
DATASET=rectangle

SCRIPT_PATH="$( cd "$(dirname "$0")" ; pwd -P )"
MAIN_DIR="$SCRIPT_PATH/logs"
PROJECT_NAME=mas_raw_dataset
SESSION_NAME=mas_raw_dataset
...
'
# play the whole bag
  'rosbag_play' './waitForRos.sh; rosbag play ../../bag_files/raw/'"$DATASET"'.bag --clock -r 1.0
'
# convert the ouster LiDAR packets into PointClouds 
  'ouster_to_pc' './waitForRos.sh; roslaunch os_replay.launch replay:=true metadata:=`pwd`/10.10.20.90.json
'
# run rviz
